document.addEventListener("DOMContentLoaded", () => {
  ...
  const hideCards = () => {
    ...
    if (matchedCards.length >= 2) {
      ...
      const second = matchedCards.filter((card) => card.dataset.emoji == first.dataset.emoji)[1];
      ...
      setTimeout(() => {
        ...
        first.classList.remove("matched");
        second.classList.remove("matched");
        first.addEventListener("click", handleCardClick);
        second.addEventListener("click", handleCardClick);
      }, 500);
    }
  };
